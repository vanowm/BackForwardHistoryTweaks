...
var bfht = {
	...
	prefs: {
		...
		overflow: {default: OVERFLOW_NONE, value: OVERFLOW_NONE, min: 0, max: 2}, //show scrollbars = 0 none, 1 = scrollbars, 2 = up/down buttons
		...
		browser_sessionhistory_max_entries: {default: 50, value: 50, min: 0}, //browser.sessionhistory.max_entries
		browser_sessionhistory_max_entries_original: {default: 50, value: 50, min: 0}, //browser.sessionhistory.max_entries
		showChangesLog: {default: true, value: true}, //show changes log after update
		version: {default: "", value: "", regexp: /[^0-9a-z.]/i},
	},
			...
			var document = window.document,
					...
					addonOptionsDisplayed = {
						...
						observe: function(document, aTopic, aData)
						{
							...
							function setShowIndexTotal()
							{
								$(document, "bfht_showIndexTotal").setAttribute("disabled", bfht.prefs.showIndex.value == 0);
							};
...
//							$(document, "bfht_showChangesLog_button").className = "text-link";
							...
							listen(window, $(document, "bfht_showIndex"), "command", setShowIndexTotal, false);
...
//      item.setAttribute("tooltiptext", tooltipForward);
	...
	item = (bfht.prefs.order.value ? end - index - 1 : index) + Math.floor(bfht.prefs.num.value > 1 ? bfht.prefs.num.value / 2 : 0);
  ...
  return true;
			...
			_dump(window.onpopstate);
listen(window, window, "popstate", function(event)
{
	_dump(1);
	_dump(event, 1);
}, true);
